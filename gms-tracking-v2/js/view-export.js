...
import { el, fill, money, monthLabel, toast, tip } from './dom.js';
      ...
      el('header', {},
        el('div', { style: { flex: 1 } },
          el('h3', {}, 'Export to Excel',
            tip('Files are stamped with today’s date, so versions never overwrite each other.'))),
...
function clientPicker(clients, redraw) {
  ...
  return el('div', { class: 'field' },
    ...
    el('span', {}, c.name)))),
    tip('Ticking more than one produces separate files — a client report never contains another client’s numbers.', 'Client export separation'));
}
