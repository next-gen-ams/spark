...
import { el, toast, dateAu, tip } from './dom.js';
...
function panel(title, sub, ...body) {
  return el('div', { class: 'panel' },
    el('header', {}, el('div', {}, el('h3', {}, title, tip(sub, `${title} help`)))),
    ...body);
}
  ...
  return panel('Campaigns',
    ...
    ended.length
      ? el('div', { class: 'body', style: { display: 'flex', gap: '10px', alignItems: 'center' } },
        tip(`${ended.length} campaign${ended.length > 1 ? 's have' : ' has'} finished. Removing them keeps the dashboard to what is actually running. Export a backup first if you want the history.`, 'Finished campaign cleanup'),
        el('button', {
          class: 'btn sm',
          onclick: () => confirmDanger({
            onBackup: exportBackup,
          ...
          el('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '8px' } },
            ...vocab(kind).map((v) => el('span', { class: 'tag' }, v,
              el('button', {
                ...
                'aria-label': `Remove ${v} from the list`,
                onclick: () => {
...
function data(rerender) {
  ...
  return panel('Data', 'Local backups. Keep one before any big import. This build stores data in your browser.',
    el('div', { class: 'body', style: { display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' } },
      ...
      tip('Reloads the three reference media plans with made-up spend', 'Sample data help'),
      el('div', { style: { flex: 1 } })),
    ...
    el('div', { class: 'body', style: { paddingTop: 0, display: 'flex', gap: '12px', alignItems: 'center' } },
      tip('Delete all data clears every client, campaign, line and spend row both here and in the shared database. FX rates, dropdown lists and column mappings are kept.', 'Delete all data impact'),
      el('button', {
        class: 'btn sm',
        onclick: () => {
