...
import { el, money, pct, monthLabel, tip } from './dom.js';
...
function tagline(state) {
  return el('div', { class: 'body', style: { paddingTop: 0 } },
    tip(`Chart covers the whole plan. The cards and the list below are ${monthLabel(state.ym)} only.`, 'Chart scope'));
}
...
function card({ k, v, s, bar, color, cls = '', title }) {
  const node = el('div', { class: 'card ' + cls },
    el('div', { class: 'k' }, k, title ? tip(title, `${k} definition`) : null),
    el('div', { class: 'v' }, v),
    el('div', { class: 's' }, s),
