...
import { el, money, pct, int, dateAu, toast, monthLabel, tip } from './dom.js';
...
function mappingPanel(sheet, rerender) {
  ...
  const row = (f) => {
    ...
    const [label, kind, tipText] = m ? (SOURCE[m.source] || ['Set', '', '']) : ['Not found', 'crit', ''];
    ...
    return el('tr', { class: bad ? 'nb' : '' },
      el('td', {},
        ...
        f.help ? tip(f.help, `${f.label} help`) : null),
      el('td', {}, el('span', { class: 'tag ' + kind }, label), tip(m?.why || tipText, `${label} explanation`)),
      el('td', { class: 'wrap' },
        el('select', {
    ...
    el('header', {},
      el('div', {},
        ...
        tip(blocked
          ...
          : 'These were worked out from the numbers rather than the headers. Worth a glance before importing.', 'Column mapping help')),
      el('div', { style: { flex: 1 } }),
      el('button', {
        class: 'btn sm',
...
function dropZone(rerender) {
  ...
  const zone = el('div', { class: 'drop' },
    el('strong', {}, 'Drop a media plan here',
      tip('Use the INT_ GMS QUOTATION / IO workbook so booked margin comes across.')),
    ...
    el('header', {},
      el('div', {},
        el('h3', {}, 'Import a media plan',
          tip('Lines, monthly budgets, booked rates and margin are read straight off the plan.')))),
    el('div', { class: 'body' }, zone,
      tip('Nothing is saved until you press Import. Subtotal rows, top-up rows and fee lines are handled for you, and you will see exactly what was skipped and why.', 'Import safety')));
}
  ...
  return el('div', { class: 'panel' },
    el('header', {}, el('div', {},
      el('h3', {}, `${parsed.sheets.length} plan sheets in this workbook`,
        tip('The best-reconciling sheet is pre-selected. Older or partial versions usually fail the check.')))),
  ...
  return el('div', { class: 'panel' },
    el('header', {}, el('div', {},
      el('h3', {}, 'Fill in what the plan left out',
        tip('These are saved with the campaign when you import. Anything left blank keeps the documented fallback.')))),
  ...
  return el('div', { class: 'panel' },
    el('header', {}, el('div', {},
      el('h3', {}, 'Where this lands',
        tip('Pick the client, and the fallback currency your team will type spend in.')))),
    el('div', { class: 'body' },
      el('div', { class: 'row2' },
        el('div', { class: 'field' },
          el('label', {}, 'Client'),
        ...
        el('div', { class: 'field' },
          ...
          tip('Fallback only. Lines decide for themselves: a China platform is typed in RMB, an international rep buy in AUD. This covers lines with neither signal. Every derived figure is AUD.', 'Spend currency help')))));
}
...
function rowTable(s, rerender) {
  ...
  return el('div', { class: 'panel' },
    el('header', {},
      el('div', {},
        el('h3', {}, `${on} lines will be imported`, tip(s.rows.length > on
          ...
          : 'Untick anything you do not want tracked. Greyed rows are fee or production lines: tracked, but kept out of pacing and the client report.', 'Import line selection')))),
    el('div', { class: 'tablewrap' }, el('table', { class: 'data' },
      el('thead', {}, el('tr', {},
...
function actions(s, ctx) {
  ...
  return el('div', { style: { display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'flex-end', marginBottom: '30px' } },
    ...
    blocked ? tip('Map the required columns first: importing half a plan puts wrong numbers in front of a client.', 'Why import is blocked') : null,
